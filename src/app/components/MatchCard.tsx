...
interface MatchCardProps {
  ...
  winProbability?: number; // 0-100 for Home team usually, or just a generic "confidence"
...
export function MatchCard({
  ...
  return (
    <Card
      className={cn(
        "group relative overflow-hidden transition-all duration-300 hover:border-[#00FF88]/50 hover:shadow-lg hover:shadow-[#00FF88]/10 cursor-pointer bg-slate-900",
    ...
    >
        ...
        {(hasAiPrediction || hasExpertPrediction) && (
          <div className="bg-slate-950/80 rounded-lg p-3 border border-slate-800/80 shadow-inner group-hover:border-[#00FF88]/20 transition-colors">
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm font-black text-white italic tracking-tight">{predictionType || "Análise Disponível"}</span>
      ...
      <CardFooter className="px-4 pb-4 pt-0">
        <Button className="w-full group-hover:bg-[#00FF88] group-hover:text-black transition-all" variant="outline">
